...
map.getView().fit([-6539482.924467, -4129312.436366, -6476187.179234, -4092070.095163], map.getSize());


/* ================================================================
 * CONFIGURACION PERSONALIZADA DEL MAPA
 *
 * BASE:
 *   - Comienza apagada.
 *   - No es interactiva (sin popup/seleccion).
 *   - Al encender BASE se muestran tambien las etiquetas del callejero.
 *
 * SUMIDEROS:
 *   - Comienzan apagados.
 *   - Mantienen la interaccion y los popups originales.
 *   - Se pueden encender individualmente desde el selector.
 * ================================================================ */
(function configurarCapasIniciales() {
    var baseLayers = [
        lyr_COMUNAS_CABA_5,
        lyr_caba_manzanas_6,
        lyr_espaciosverdescatastralesespaciosverdescatastro_espacios_verdes_08042020shp_7,
        lyr_espacioverdeprivadoespacio_verde_privado_wgs84shp_8,
        lyr_callejeroetiqueta_9
    ];
    ...
    // BASE: siempre pasiva, nunca popup/seleccion.
    baseLayers.forEach(function(layer) {
        layer.set('interactive', false);
        layer.setVisible(false);
    });

    // SUMIDEROS: interactivos, pero inicialmente apagados.
    sumideroLayers.forEach(function(layer) {
        layer.set('interactive', true);
        layer.setVisible(false);
    });

    // Los dos grupos tambien comienzan apagados.
    group_BASE.setVisible(false);
    group_Sumideros.setVisible(false);

    // Al encender BASE desde el selector, mostrar sus capas.
    group_BASE.on('change:visible', function(evt) {
    ...
    // Al apagar BASE, sus capas tambien quedan apagadas.
    // Si se enciende una capa individual desde el selector, OpenLayers
    // conserva esa seleccion sin volver interactiva la capa.

    // Al encender/apagar SUMIDEROS como grupo, sincronizar sus capas.
    group_Sumideros.on('change:visible', function(evt) {
        var visible = evt.target.getVisible();
        sumideroLayers.forEach(function(layer) {
            layer.setVisible(visible);
        });
